...
1. Rename some references inside the repo:
   ...
   git mv imageroot/systemd/user/patchmon.service imageroot/systemd/user/${modulename}.service &&
   git mv imageroot/systemd/user/patchmon-app.service imageroot/systemd/user/${modulename}-app.service && 
   git mv tests/patchmon.robot tests/${modulename}.robot &&
   sed -i "s/patchmon/${modulename}/g" $(find .github/ * -type f) &&
...
Instantiate the module with:
    ...
    add-module ghcr.io/nethserver/patchmon:latest 1

The output of the command will return the instance name.
Output example:

    {"module_id": "patchmon1", "image_name": "patchmon", "image_url": "ghcr.io/nethserver/patchmon:latest"}

## Configure

Let's assume that the mattermost instance is named `patchmon1`.
...
See also the `systemd/user/patchmon.service` file.
...
- The module runs under an agent that initiate a lot of environment variables (in /home/patchmon1/.config/state), it could be nice to verify them
...
Test the module using the `test-module.sh` script:
    ...
    ./test-module.sh <NODE_ADDR> ghcr.io/nethserver/patchmon:latest
